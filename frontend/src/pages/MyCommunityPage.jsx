...
import { Copy, Plus, RefreshCcw, Eye } from "lucide-react";
...
export default function MyCommunityPage() {
  ...
  const [togglingEmergency, setTogglingEmergency] = useState({});
  ...
  const toggleEmergencyMode = async (communityId) => {
    setError("");
    setTogglingEmergency((prev) => ({ ...prev, [communityId]: true }));
    try {
      const { data } = await api.post(`/communities/${communityId}/emergency-mode`);
      if (data?.community) {
        setCommunities((prev) =>
          prev.map((c) => (c._id === communityId ? { ...c, emergencyMode: data.community.emergencyMode } : c))
        );
      }
      setToast(data?.community?.emergencyMode ? "Emergency mode ON" : "Emergency mode OFF");
    } catch (err) {
      setError(err?.response?.data?.message || err?.message || "Failed to toggle emergency mode");
    } finally {
      setTogglingEmergency((prev) => ({ ...prev, [communityId]: false }));
    }
  };
                  ...
                  <div className="flex items-center justify-between gap-3">
                    <div>
                      ...
                      <p className={`text-xs font-medium ${community.emergencyMode ? "text-rose-600" : "text-slate-400"}`}>
                        {community.emergencyMode ? "ACTIVE - All members can see" : "Inactive"}
                      </p>
                    </div>
  ...
  }, [communities, error, loading, togglingEmergency]);
