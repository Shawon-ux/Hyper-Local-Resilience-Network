...
export default function CompletedTasksPage() {
  ...
  const loadTasks = async () => {
    try {
      ...
      const completed = Array.isArray(data) ? data.filter(t => t.status === 'completed') : [];
      setTasks(completed);
    } catch (err) {
                ...
                >
                  ...
                  {task.helper && (
                    <div className="mt-4 border-t border-slate-100 pt-4">
                      <p className="text-[10px] uppercase tracking-wider text-slate-400 font-bold">Helper</p>
                      <p className="text-sm font-medium text-slate-800">{task.helper.name}</p>
                    </div>
                  )}
                </div>
              ))}
