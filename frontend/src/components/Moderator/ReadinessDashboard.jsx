...
const ReadinessDashboard = () => {
  const [data, setData] = useState({ stressPoints: [], resourceGaps: [], overallReadiness: 0 });

  useEffect(() => {
    axios.get('/api/resources/moderator/readiness').then(res => setData(res.data));
  }, []);
    ...
    <div className="p-6 bg-slate-50 min-h-screen">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <div className="bg-white p-6 rounded-xl shadow-sm border-l-4 border-blue-500">
          ...
          <h2 className="text-3xl font-bold">{Math.round(data.overallReadiness)}%</h2>
        </div>
        <div className="bg-white p-6 rounded-xl shadow-sm border-l-4 border-red-500">
          <p className="text-sm text-gray-500">Critical Gaps</p>
          <h2 className="text-3xl font-bold text-red-600">{data.stressPoints.length}</h2>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-sm overflow-hidden">
        <div className="p-4 border-b bg-slate-100 font-bold flex items-center gap-2">
          <AlertTriangle className="text-orange-500 w-5 h-5" /> Potential Stress Points
        </div>
          ...
          <tbody>
            {data.stressPoints.map(item => (
              <tr key={item._id} className="border-b hover:bg-slate-50">
                ...
                <td className="p-4 text-red-600 font-bold">{item.totalStock - item.consumed} {item.unit} left</td>
                <td className="p-4">
